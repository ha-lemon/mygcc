...
public:
  enum TypeID {
    VoidTyID,     // Void
    LabelTyID,    // Labels, e.g., BasicBlock          用于给基本块
...
public:
  explicit Value(Type *ty, const std::string &name = "")
      : type_(ty), name_(name) {}
  ~Value() = default;
  virtual std::string print() = 0;    //返回可读形式的代码
...
public:
  ...
  bool is_declaration() { return basic_blocks_.empty(); }   //引用说明？还是定义？
  void set_instr_name();
  void remove_bb(BasicBlock *bb);
  BasicBlock *getRetBB();   //获取返回处的基本快的指针。

  std::vector<BasicBlock *> basic_blocks_; // basic blocks 所含的基本快
  std::vector<Argument *> arguments_;      // argument 参数
  Module *parent_;   //所属的Module
  unsigned seq_cnt_;
...
public:
  ...
  //********************使用list替换vector---------
  std::list<Instruction *> instr_list_; //指令集合
  //********************使用list替换vector---------
