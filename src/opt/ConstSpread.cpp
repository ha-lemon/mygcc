  ...
  for (auto bb : func->basic_blocks_) {
    ...
    for (auto instr : bb->instr_list_) {
      ...
      case Instruction::ICmp:
        ...
        testConstIntb = dynamic_cast<ConstantInt *>(instr->get_operand(1));
        if (testConstInta && testConstIntb) {
          auto res = this->CalcICMP(dynamic_cast<ICmpInst *>(instr)->icmp_op_,   //强转为ICMPInst再求其icmp_op_
  ...
  for (auto bb : func->basic_blocks_) {
    ...
    if (br->op_id_ == Instruction::Br &&
      ...
      if (cond->value_ == 0) {
        ...
        bb->succ_bbs_.clear();
        new BranchInst(dynamic_cast<BasicBlock *>(falsebb), bb);   //bb->falsebb 条件跳转变为无条件跳转
      } else {
        bb->delete_instr(br);
        for (auto succ_bb : bb->succ_bbs_) {
          succ_bb->remove_pre_basic_block(bb);
